...
@section('content')
    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                            ...
                            <tbody>
                            @foreach ($employeeSituations as $item)
                                <tr>
                                    <td>{{ $loop->index+1 }}</td>
                                     <td>{{ $item->name }}</td>
                                ...
                                @include('pages.EmployeeSituations.edit')

                                <!--  page of delete_modal_city -->
                                @include('pages.EmployeeSituations.delete')


                            @endforeach
                        </table>

                        <div> {{$employeeSituations->links('pagination::bootstrap-4')}}</div>
                    </div>
                </div>
            </div>
        </div>
       ...
       @include('pages.EmployeeSituations.create')
    </div>
